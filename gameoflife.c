...
int **alocaMatriz(int l, int c){ //Função para alocar a matriz de acordo com a linha e a coluna especificada no arquivo de entrada
	int **m, i;

	if(l < 1 || c < 1){ //Como a entrada pode ser de 0 a 100, e não existe matriz com nenhuma dimensão igual a 0, deve ser feito um tratamento de erros
...
int **liberaMatriz(int l, int c, int **m){ //Função para liberar o espaço alocado para a(s) matriz(es)
...
void imprime(int **matriz, int l, int c){ //Função para imprimir na tela a geração atual
...
int main(){
	...
	while(1){
		escolha = pergunta(); //Interação com o usuário sobre o avanço da geração
			...
			for(i = 1; i <= lin; i++){ //Comparação de cada posição da matriz e de seus vizinhos, gerando uma outra matriz com o número de vizinhos
				for(j = 1; j <= col; j++){
					cont[i][j] = 0;
					if(pr[i][j] == 1)
			...
			for(i = 1; i <= lin; i++){ //Análise de qual célula é viva ou morta à partir das regras impostas pelo "Jogo da Vida"
				for(j = 1; j <= col; j++){
					if(cont[i][j] == 0) //Regra 1
						pr[i][j] = 0;

					else if(cont[i][j] == 1) //Regra 2
						pr[i][j] = 0;

					else if(cont[i][j] == 3 && pr[i][j] == 0) //Regra 3
						pr[i][j] = 1;

					else if(cont[i][j] > 3 && pr[i][j] == 1) //Regra 4
						pr[i][j] = 0;
				}
			}
